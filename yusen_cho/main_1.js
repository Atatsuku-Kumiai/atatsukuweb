...
Papa.parse('data/nara_purchases.csv', {
  ...
  complete: function(results){
    ...
    const rankingData = Object.entries(municipalityData)
      .filter(([name, v]) => name && name !== "null")   // ★ここを追加
      .map(([name, v]) => ({
        name: name,                       // ←「（不明）」は付けない
        total: (v.goods||0) + (v.services||0)        // ★金額が無い場合は棒グラフが0で表示                                
      }))
